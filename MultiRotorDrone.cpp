...
MultiRotorDrone::MultiRotorDrone():Drone(),random(0) {

}

MultiRotorDrone::MultiRotorDrone(MultiRotorDrone &drone):Drone(drone.getPosition().getX(),drone.getPosition().getY(),drone.getVelositi().getX(),drone.getVelositi().getY(),drone.getPersonalBest(),drone.getType()),random(0){

}

MultiRotorDrone::MultiRotorDrone(double x, double y, double velx, double vely, Vector2f vector, char t, int rand):Drone(x, y,velx,vely,vector,t),random(rand) {

}

void MultiRotorDrone::update() {
    if(random==0){
...
std::ostream &MultiRotorDrone::print(std::ostream &out) const {
    out<<getType()<<" ";
...
MultiRotorDrone &MultiRotorDrone::operator=(const MultiRotorDrone &drone) {
    if(this==&drone){
        return *this;
    }
