...
FixedWingDrone::FixedWingDrone():Drone(),random(0){

}

FixedWingDrone::FixedWingDrone(FixedWingDrone &drone):Drone(drone.getPosition().getX(),drone.getPosition().getY(),drone.getVelositi().getX(),drone.getVelositi().getY(),drone.getPersonalBest(),drone.getType()),random(0) {

}

FixedWingDrone::FixedWingDrone(double x, double y, double velx, double vely, Vector2f vector, char t, int rand):Drone(x, y,velx,vely,vector,t),random(rand) {

}

void FixedWingDrone::update() {
    if(random==0){
...
std::ostream &FixedWingDrone::print(std::ostream &out) const {
    out<<getType()<<" ";
...
FixedWingDrone& FixedWingDrone::operator=(const FixedWingDrone &drone) {
    if(this==&drone){
        return *this;
    }
